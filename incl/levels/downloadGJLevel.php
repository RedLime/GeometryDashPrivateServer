...
if(!is_numeric($levelID)){
	...
	switch($levelID){
		...
		case -2:
			...
			$query->execute([':time' => time()]);
			$result = $query->fetch();
			if (!isset($result["levelID"]) || !isset($result["feaID"])) {
				$daily = 0;
				break;
			}
			$levelID = $result["levelID"];
