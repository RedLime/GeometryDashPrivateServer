...
foreach($result as &$comment1) {
	if($comment1["commentID"]!=""){
		...
		if ($query12->rowCount() > 0) {
			...
			$prefixed = $gs->getPowerRoleName($extID, $userName);
			if(!in_array($user["userID"], $users)){
				$users[] = $user["userID"];
				$userstring .=  $user["userID"] . ":" . $userName . ":" . $extID . "|";
			}
			if($binaryVersion > 31){
				$commentstring .= "~11~".$gs->getMaxValuePermission($extID, "modBadgeLevel")."~12~".$gs->getAccountCommentColor($extID).":1~".$prefixed.$userName."~7~1~9~".$user["icon"]."~10~".$user["color1"]."~11~".$user["color2"]."~14~".$user["iconType"]."~15~".$user["special"]."~16~".$user["extID"];
			}
			$commentstring .= "|";
		}
	}
