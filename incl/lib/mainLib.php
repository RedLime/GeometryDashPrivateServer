...
class mainLib {
	...
	public function getMaxValuePermission($accountID, $permission){
		...
		$roleIDlist = substr($roleIDlist, 0, -1);
		if($roleIDlist != ""){
			$query = $db->prepare("SELECT $permission FROM roles WHERE isDefault != 1 AND roleID IN ($roleIDlist) ORDER BY priority DESC");
			$query->execute();
			$roles = $query->fetchAll();
			foreach($roles as &$role){ 
	...
	public function getPowerRoleName($accountID, $username){
		include __DIR__ . "/connection.php";
		$maxvalue = 0;
		$query = $db->prepare("SELECT roleID FROM roleassign WHERE accountID = :accountID");
		$query->execute([':accountID' => $accountID]);
		...
		$roleIDlist = substr($roleIDlist, 0, -1);
		if($roleIDlist != ""){
			$query = $db->prepare("SELECT rolePrefix FROM roles WHERE isDefault != 1 AND roleID IN ($roleIDlist) ORDER BY priority DESC");
			$query->execute();
			$roles = $query->fetchAll();
			foreach($roles as &$role){ 
				if($role["rolePrefix"] > $maxvalue){
					$maxvalue = $role["rolePrefix"];
				}
			}
		}
		if ($maxvalue == 1) {
	...
	public function getAccountCommentColor($accountID){
		...
		$roleIDlist = substr($roleIDlist, 0, -1);
		if($roleIDlist != ""){
			$query = $db->prepare("SELECT commentColor FROM roles WHERE isDefault != 1 AND roleID IN ($roleIDlist) ORDER BY priority DESC");
			$query->execute();
			$roles = $query->fetchAll();
			foreach($roles as &$role){
	...
	public function rateLevel($accountID, $levelID, $stars, $difficulty, $auto, $demon, $ignore = false){
		include __DIR__ . "/connection.php";
	...
	public function voteLevel($accountID, $levelID, $difficulty, $demon){
		...
		$query = $db->prepare("SELECT isRateBanned FROM accounts WHERE accountID = :accountID");
		$query->execute([':accountID' => $accountID]);
