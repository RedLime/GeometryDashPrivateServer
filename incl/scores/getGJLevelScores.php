...
$query2 = $db->prepare("SELECT percent, coins FROM levelscores WHERE accountID = :accountID AND levelID = :levelID");
$query2->execute([':accountID' => $accountID, ':levelID' => $levelID]);
$res = $query2->fetch();
$oldPercent = $res["percent"];
$oldCoins = $res["coins"];
if($query2->rowCount() == 0) {
	...
	if($oldPercent < $percent || $coins > $oldCoins){
...
switch($type){
	case 0:
		...
		$query2 = $db->prepare("SELECT accountID, uploadDate, percent, coins FROM levelscores WHERE levelID = :levelID AND accountID IN ($friends) ORDER BY percent DESC, coins DESC, uploadDate ASC");
		$query2args = [':levelID' => $levelID];
		break;
	case 1:
		$query2 = $db->prepare("SELECT accountID, uploadDate, percent, coins FROM levelscores WHERE levelID = :levelID ORDER BY percent DESC, coins DESC, uploadDate ASC");
		$query2args = [':levelID' => $levelID];
		break;
	case 2:
		$query2 = $db->prepare("SELECT accountID, uploadDate, percent, coins FROM levelscores WHERE levelID = :levelID AND uploadDate > :time ORDER BY percent DESC, coins DESC, uploadDate ASC");
...
foreach ($result as &$score) {
	...
	$user = $user[0];
	$time = $gs->timeDelay($score["uploadDate"]);
	if($user["isBanned"]==0){
		...
		echo "1:".$user["userName"].":2:".$user["userID"].":9:".$user["icon"].":10:".$user["color1"].":11:".$user["color2"].":14:".$user["iconType"].":15:".$user["special"].":16:".$user["extID"].":3:".$score["percent"].":6:".$rank.":13:".$score["coins"].":42:".$time."|";
	}
}
?>